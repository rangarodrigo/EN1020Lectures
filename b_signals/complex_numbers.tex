...
\begin{frame}{Representing Complex Numbers}
    \begin{columns}[T]
        ...
        \pause
        \begin{column}{0.5\textwidth}
            \noindent The relationship between these two representations can be determined from \alert{Euler's formula}:
            \begin{equation*}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.5\textwidth}
            \mode<beamer>
            {
                ...
                \onslide<3-|handout:0>
                \begin{align*}
                    z_3 &= r_0e^{j(\theta_0 + \pi)}\\&= r_0(\cos(\theta_0 + \pi) + j\sin(\theta_0 + \pi)) = -x_0 - jy_0 = -z_0.
                \end{align*}
                \onslide<4-|handout:0>
                \begin{equation*}
                    z_4 = -x_0 + jy_0.
    ...
    \begin{columns}[T]
        ...
        \onslide<1>
                ...
                \begin{tikzpicture}
                    ...
                    \draw[red!50!black, thick] (0,0) -- (60:1.5) node[midway, above, yshift=0.1cm] {$2$} node[anchor=west] {$1 + j\sqrt{3}$};
...
\begin{frame}{$\cos\theta$ and $\sin\theta$}
    Using Euler's fromula, derive the following relationships:
    \begin{enumerate}
