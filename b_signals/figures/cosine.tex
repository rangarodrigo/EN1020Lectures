\pgfplotsset{compat=1.12}
\pgfplotsset{every tick label/.append style={font=\scriptsize}}
  \begin{tikzpicture}
  \def\omwga0{2*pi}
  \def\phase{-30}
    \begin{axis}[
     clip=false,
     xmin=-1.5*pi,xmax=3.5*pi,
     xlabel= $t$,
     ylabel=$ x(t)$,
     ymin=-1.5,ymax=1.5,
     axis lines=middle,
     %axis x line=middle,
     %axis y line=left,
%     axis x line=middle,
     xtick={-3.14, -1.57, 0,1.57,3.14,4.71,6.28, 7.85},
     xticklabels={$-\pi$, $-\frac{\pi}{2}$, $0$, $\frac{\pi}{2}$,$\pi\,$,$\,\,\,\frac{3}{2}\pi$,$\,\,\,2\pi$, $\frac{5\pi}{2}$},
     ytick={-1, 1},
     yticklabels={$\small  -A$, $\small  A$},
     %xticklabel style={anchor=north west}
        x label style={at={(current axis.right of origin)},anchor=west},
        y label style={at={(current axis.above origin)}, anchor=south},
     ]
      \addplot[domain=-pi:3*pi,samples=200,EntcMaroonDef, thick]{cos(deg(x) + \phase)}
                                node[right,pos=1,font=\footnotesize, black]{\scriptsize $x(t)=A\cos (t - \pi/6)$};



       \node at (axis cs:0, .8667) [anchor=east] {\scriptsize $A\cos\phi$  };

       \node at (axis cs:2.5*pi, 1.2) [anchor=west] {\scriptsize $A =$ amplitude };
       \node at (axis cs:2.5*pi, 1.0) [anchor=west] {\scriptsize $\omega_0 =$ frequency (here $\omega_0 =1$) };
       \node at (axis cs:2.5*pi, 0.8) [anchor=west] {\scriptsize $\phi =$ phase (here $\phi =-\pi/6$) };
       \draw[|-|] (axis cs: pi/6,1.1) -- (axis cs: 2*pi +pi/6,1.1) node[midway, above] {\scriptsize $T_0=\frac{2\pi}{\omega_0}$};
       \draw[dashed] (axis cs:0.167*pi, 1.0) -- (axis cs:0.167*pi, -0.5) node [anchor=north, EntcBlue] {\scriptsize $\frac{\pi}{6}$ };
       \draw[dashed] (axis cs:2.167*pi, 1.0) -- (axis cs:2.167*pi, -0.5) node [anchor=north, EntcBlue] {\scriptsize $2\pi + \frac{\pi}{6}$ };
    \end{axis}


  \end{tikzpicture} 