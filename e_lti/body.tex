...
\begin{frame}{Systems with and without Memory}
    A system is said to be \alert{memoryless}  if its output for each value of the independent variable at a given time is dependent only on the input at the same time instant.
    \vspace{0.1in}
    \par
    \begin{columns}
        \column{0.48\textwidth}
            ...
            \begin{equation*}
                y(t) = Rx(t),
            \end{equation*}
            where $x(t)$ the current through the resistor $R$ and $y(t)$ taken as the voltage across the resistor.
            \begin{equation*}
                y(t) = x(t),
            \end{equation*}
    ...
    \mode<beamer>
    {
        \begin{tikzpicture}[remember picture,overlay]
            \node[anchor=east, yshift=-0.2in,  rounded corners=2pt, fill=blue!10!white, opacity=.5,text opacity=1] at (current page.east)
...
\begin{frame}{Invertibility and Inverse Systems}
    A system is \alert{invertible} if distinct inputs lead to distinct outputs (one-to-one). If a system is invertible, the \alert{inverse system} exists, and when cascaded with the original system, yields an output equal to the input to the first system.
    \vspace{0.1in}
    \par
    \mode<beamer>
    {
        \centering
        \begin{tikzpicture}[node distance=4cm, block/.style = {rounded corners=2pt, thin, draw=EntcBlue, align=center, text width=3cm, fill=blue!10!white}]
            \node (a) at (0,0) {$x[n]$};
            \node (b) [right= 1cm of a, thin, block] {System};
            \node (c) [right= 1cm of b, thin,  block] {Inverse system};
            \node (d) [right= 1cm of c] {$w[n] = x[n]$};
            \draw[-latex, thin, EntcBlue] (a) -- (b);
            \draw[-latex, thin, EntcBlue, text=black] (b) -- (c) node[midway, above] {$y[n]$};
            \draw[-latex, thin, EntcBlue] (c) -- (d);
        \end{tikzpicture}
    }
    \mode<handout>
    {
    ...
    \begin{columns}
        ...
        The differentiator is not invertible in general because the constant value (the DC offset) is lost during differentiation. Therefore, the integrator is not a true inverse system unless additional information (e.g., initial conditions) is available.
        \begin{equation*}
            \text{Inte.: } y_1(t) = \int_{-\infty}^{t}x_1(\tau)d\tau \quad
            \text{Diff.: } y_2(t) = \frac{dx_2(t)}{dt}
        \end{equation*}
        % \noindent\textbf{Examples of non-invertible systems:}\par
        \begin{equation*}
            y[n] = 0.
...
\begin{frame}{Stability}
    ...
    For example, if $x(t) =1$  which is bounded, then $y(t)=t$, which is unbounded; hence the system is not BIBO stable.    
    }
    \pause
...
\begin{frame}{Time Invariance}
    ...
    \mode<beamer>
    {
        \begin{columns}
            ...
            \end{equation*}  
            Since the multiplying function $\sin t$ depends explicitly on time, the system is time-varying.                                  
        \end{columns}
    }
\end{frame}
...
\begin{frame}{Strategy for Exploiting Linearity and Time Invariance}
    ...
    \begin{itemize}
        ...
        \item   Choose basic signals so that the response is easy to compute (analytical convenience).
    \end{itemize}
...
\begin{frame}{A DT Signal as  Superposition of Weighted Delayed Impulses}
    \begin{itemize}
      ...
      \item If we have a linear system, and a signal expressed as above as a linear combination of basic signals, the response would be the same linear combination  the responses for individual basic signals.
    \end{itemize}
...
\begin{frame}{Convolution Sum}
    ...
    \pause
    \begin{align*}
        &h_k[n] = h_0[n-k] \quad \text{where }h_0[n] \text{ is the response of the system for an impulse at 0, i.e., } \delta[n].\\
        &\text{We define } h_0[n] = h[n].
    \end{align*}    
    \pause
    If syten is both linear and time invariant (LTI)
    \begin{equation*}
...
\begin{frame}{Example}
    Compute $y[n] = x[n]\ast h[n]$ for $x[n]$ and $h[n]$ as shown in Figure \ref{fi:example01_discrete_conv_signals}.
    %\mode<beamer>
    {
    \begin{figure}
      \centering
...
\begin{frame}{The Continuous-Time Unit Impulse Response and the Convolution Integral Representation of LTI Systems}
    ...
    \end{equation*}
    Since the pulse $\delta_\Delta(t - k\Delta)$ corresponds to a shifted unit impulse as $\Delta \rightarrow 0$, the response $\hat{h}_{k\Delta}(t)$ to this input pulse becomes the response to an impulse in the limit. If we let $h_\tau(t)$ denote the response at time $t$ to a unit impulse $\delta(t - \tau)$ located at time $\tau$, then
    \begin{equation*}
    ...
    {
        \begin{tikzpicture}[remember picture,overlay]
            \node[anchor=east, yshift=-1.2in, xshift=-1in, fill=blue!20!white, opacity=.8,text opacity=1] at (current page.east)
            {$x(t) = \int_{-\infty}^{\infty}x(\tau)\delta(t-\tau)d\tau$};
        \end{tikzpicture}
    }
...
\begin{frame}{}
    ...
    {
            \begin{tikzpicture}[remember picture,overlay]
                    \node[anchor=north east, yshift=0in, xshift=0in, fill=blue!20!white, opacity=.8,text opacity=1] at (current page.north east)
                    {
                        \begin{tikzpicture}
                            \node (xt) at (0,0)  {$x(t)$};
                            \node (ht) [right of=xt, draw, fill=red!20!white] {$h(t)$};
                            \node (yt) [right of=ht] {$y(t)$};
                            \draw[-latex] (xt) -- (ht);
                            \draw[-latex] (ht) -- (yt);
                        \end{tikzpicture}
    ...
    {
            \begin{tikzpicture}[remember picture,overlay]
                    \node[anchor=north east, yshift=-0.25in, xshift=0in, fill=blue!20!white, opacity=.8,text opacity=1] at (current page.north east)
                    {
                        $
                        \begin{aligned}
                        y(t) &= x(t) \ast h(t) \\
                            &= \int_{-\infty}^{\infty} x(t) \ast h(t - \tau)d\tau \\
                            &= \int_{-\infty}^{\infty} x(\tau) \delta(t - \tau - t_0) d\tau \\
                            &= \int_{-\infty}^{\infty} x(\tau) \delta((t - t_0) - \tau) d\tau \\
                            &= x(t - t_0)\int_{-\infty}^{\infty} \delta((t - t_0) - \tau) d\tau \\
                            &= x(t - t_0)
                        \end{aligned}
                        $
                    };
...
\begin{frame}{Convolution}
    \begin{enumerate}
      ...
      \item The convolution sum and the convolution integral resulted out of the particular choice of the basic signals, delayed unit impulses.
