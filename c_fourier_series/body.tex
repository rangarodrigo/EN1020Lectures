...
\begin{frame}{Introduction}
    \begin{itemize}[<+->]
      ...
      \item We use the Fourier series for periodic signals and the Fourier transform for aperiodic signals.
      \item Each of these has continuous-time and discrete-time versions:
        \begin{enumerate}
        ...
        \end{enumerate}
      \item In this part of the course, we will focus on computing the continuous-time Fourier series and Fourier transform. Later, after covering linear time-invariant (LTI) systems, we will explore the conceptual aspects of Fourier techniques.
    \end{itemize}
\end{frame}
    ...
    \begin{columns}[t]
        ...
        \begin{column}{0.6\textwidth}
            \begin{itemize}[<+->]
              ...
              \item Periodic signals have a line spectrum, called the Fourier series.
...
\begin{frame}{Continuous-Time Fourier Series}
    ...
    \begin{tikzpicture}[remember picture,overlay]
        \node[draw=EntcRed, fill=red!5!white, inner sep=2pt,outer sep=2pt, rounded corners=0.1cm, anchor=south east, yshift=6.5cm, xshift=0cm, text width=4cm, align=left]  at (current page.south east) 
        {
            \begin{equation*}
                \begin{aligned}
...
\begin{frame}[plain]
    \begin{example}
        ...
        which has the fundamental frequency $\omega_0$.
        \begin{enumerate}
            \item Use Euler's formula to express $x(t)$ as a linear combination of complex exponentials.
...
\begin{frame}<beamer>[plain,t]
        ...
        \begin{tikzpicture}[remember picture,overlay]
          \node[draw=blue!50, fill=blue!20, inner sep=2pt,outer sep=2pt, rounded corners=0.1cm, anchor=south east, yshift=6.7cm, xshift=0cm, text width=3cm, align=left]  at (current page.south east) {Euler's formula\\$e^{j\theta} = \cos\theta + j\sin\theta$\\$\cos \theta = \frac{e^{j\theta} + e^{-j\theta}}{2}$\\$\sin \theta = \frac{e^{j\theta} - e^{-j\theta}}{2j}$} ;
        \end{tikzpicture}
          ...
          \begin{column}{0.5\textwidth}
                ...
                \begin{equation*}
                    \begin{split}
                       ...
                       &= \frac{1}{T}\left[\frac{e^{-jk\omega_0 t}}{-jk\omega_0}\right]^{T_1}_{-T_1}
                    \end{split}
                \end{equation*}
          \end{column}
          \begin{column}{0.5\textwidth}
...
\begin{frame}{Properties of the Continuous-Time Fourier Series}
    Suppose that $x(t)$ is periodic signal with period $T$ and fundamental frequency $\omega_0 = 2\pi/T$. If the Fourier series coefficients are denoted by $a_k$, then
    \begin{equation}
...
\begin{frame}<beamer>
    \begin{itemize}[<+->]
      \item Time reversal applied to a continuous-time signal results in a reversal of the indices of the corresponding Fourier series coefficients.
...
\begin{frame}[plain]\frametitle{Time Scaling}
    ...
    \mode<beamer>
    {
        ...
        While Fourier coefficients have not changed, the Fourier series representation \alert{has} changed because of the change in the fundamental frequency.
        ...
        \begin{column}{0.5\textwidth}
            ...
            \begin{tikzpicture}
                \begin{axis}[
                    ...
                    \addplot+[ycomb, EntcRed, thick] coordinates {(-1,1/2) (1,1/2)};
 ...
 \mode<beamer>
{
    ...
    \pause
    \begin{equation*}
        a_k = \begin{cases} \frac{1}{jk\pi}, & k \;\text{odd},\\ 0,& k \;\text{even}.\end{cases}
    \end{equation*}
    \pause
    \begin{equation*}
...
\begin{frame}{Convergence of Fourier Series}
    ...
    \begin{tikzpicture}[remember picture,overlay]
    \node[draw=blue!50, fill=blue!20, inner sep=2pt,outer sep=2pt, rounded corners=0.1cm, anchor=south east, yshift=5.5cm, xshift=-1cm, text width=5cm]  at (current page.south east) {
...
\begin{frame}{Gibbs Phenomenon}
    \begin{itemize}
    \item When a function with \alert{jump discontinuities} is approximated by a \alert{finite number of Fourier series terms}, oscillations appear near the discontinuities.
    \item These oscillations:
    \begin{itemize}
    \item do not disappear as the number of harmonics increases,
    \item become more localized around the discontinuity,
    \item have a maximum overshoot of approximately 9\% of the jump magnitude.
    \end{itemize}
    \item This effect is known as the \alert{Gibbs phenomenon}.
    \item Away from the discontinuities, the Fourier series converges to the original signal.
    \end{itemize}

    \vspace{0.3cm}
    \mode<beamer>
    {    
    Increasing the number of Fourier coefficients improves global accuracy but cannot eliminate local overshoot near discontinuities.
    }
\end{frame}
...
\begin{frame}{Gibbs Phenomenon}
    \begin{figure}
      \centering
      \input{figures/gibbs}
      \caption{Gibbs phenomenon}\label{fi:gibbs_phenomenon}
    \end{figure}
\end{frame}


\begin{frame}<beamer>{Gibbs Phenomenon}
    \begin{figure}
      \centering
      \input{figures/gibbs2}
      \caption{Gibbs phenomenon}\label{fi:gibbs_phenomenon2}
    \end{figure}
\end{frame}
